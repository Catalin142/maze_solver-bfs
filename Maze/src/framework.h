...
namespace fm
{
	...
	struct v2
	{
		...
		inline v2 normalize() const { T inv = 1 / magnitude(); return v2(x * inv, y * inv); }
		inline const v2<T> operator* (const v2<T>& r) const { return { x * r.x, y * r.y }; }
		inline const v2<T> operator* (T value) const { return { x * value, y * value }; }
	};

	typedef v2<float> v2f;

	template <typename T>
	T dot(const v2<T>& lhs, const v2<T>& rhs) { return lhs.x * rhs.x + lhs.y * rhs.y; }

	template <typename T>
	v2<T> cross(const v2<T>& lhs, const v2<T>& rhs) { return v2<T>(lhs.x * rhs.y - lhs.y * rhs.x); }
	...
	struct v3
	{
		...
		inline v3 normalize() const { T inv = 1.0f / magnitude(); return v3(x * inv, y * inv, z * inv); }
		inline const v3<T> operator* (T value) const { return { x * value, y * value, z * value }; }
		inline const v3<T> operator/ (T value) const { return { x / value, y / value, z / value }; }
		inline const v3<T> operator+ (T value) const { return { x + value, y + value, z + value }; }
		inline const v3<T> operator+ (const v3<T>& r) const { return { x + r.x, y + r.y, z + r.z }; }
		inline const v3<T> operator- (const v3<T>& r) const { return { x - r.x, y - r.y, z - r.z }; }
		inline const v3<T> operator* (const v3<T>& r) const { return { x * r.x, y * r.y, z * r.z }; }
	};
	...
	template <typename T>
	T dot(const v3<T>& lhs, const v3<T>& rhs)
	{
		return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z;
	}

	template <typename T>
	v3<T> cross(const v3<T>& lhs, const v3<T>& rhs) 
	{ 
		return v3<T>(lhs.y * rhs.z - lhs.z * rhs.y, lhs.z * rhs.x - lhs.x * rhs.z, lhs.x * rhs.y - lhs.y * rhs.x); 
	}
	...
	struct v4
	{
		...
		inline v4 normalize() const { T inv = 1.0f / magnitude(); return v4(x * inv, y * inv, z * inv, w * inv); }
		inline const v4<T> operator* (T value) const { return { x * value, y * value, z * value, w * value }; }
		inline const v4<T> operator/ (T value) const { return { x / value, y / value, z / value, w / value }; }
		inline const v4<T> operator+ (T value) const { return { x + value, y + value, z + value, w + value }; }
		inline const v4<T> operator+ (const v4<T>& r) const { return { x + r.x, y + r.y, z + r.z, w + r.w }; }
		inline const v4<T> operator- (const v4<T>& r) const { return { x - r.x, y - r.y, z - r.z, w - r.w }; }
		inline const v4<T> operator* (const v4<T>& r) const { return { x * r.x, y * r.y, z * r.z, w * r.w }; }
	};
	...
	struct color
	{
		...
		color(const v3<float>& rgb)
		{
			hex = (int(max(rgb.r, 0.0f) * 255) << 16) | (int(max(rgb.g, 0.0f) * 255) << 8) | (int(max(rgb.b, 0.0f) * 255));
		}
	...
	public:
		...
		void set_fullscreen_buffer(void* buffer);
		...
		void draw_framebuffer(framebuffer* fm, uint32_t x, uint32_t y, uint32_t s = 1, v2<int8_t> flip = {0, 0});
...
#undef fm_def
	...
	float to_radians(float degrees)
	{
		return (degrees * 3.14159265358979323f) / 180.0f;
	}

	float to_degrees(float radians)
	{
		return (radians * 180.0f) / 3.14159265358979323f;
	}
	...
	void application::set_fullscreen_buffer(void* buffer)
	{
		memcpy(pgraphics_context->memory_buffer, buffer, pgraphics_context->buffer_width * pgraphics_context->buffer_height * sizeof(uint32_t));
	}
	...
	void application::draw_framebuffer(framebuffer* fm, uint32_t x, uint32_t y, uint32_t pixel_size, v2<int8_t> flip)
	{
		if (flip.x < 0) flip.x = -1;
		else flip.x = 1;

		if (flip.y < 0) flip.y = -1;
		else flip.y = 1;

		uint32_t pos_y = y;
		if (flip.y >= 0)
			pos_y = y + fm->height * pixel_size;

		for (uint32_t i = 0; i < fm->height; i++)
		{
			uint32_t pos_x = x;
			if (flip.x < 0)
				pos_x = x + fm->width * pixel_size;

			for (uint32_t j = 0; j < fm->width; j++)
			{
				draw_quad_fill(fm::color(fm->buffer[i * fm->width + j]),
					pos_x, pos_y, pixel_size, pixel_size);
				pos_x = pos_x + (pixel_size * flip.x);
			}
			pos_y = pos_y - (pixel_size * flip.y);
		}
	}
	...
	static LRESULT CALLBACK window_proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SIZE: 
			...
			application::app_instance->pwindow->width = width;
			application::app_instance->pwindow->height = height;
			break;
		}
	...
	void framebuffer::set_pixel(uint32_t x, uint32_t y, uint32_t value)
	{
		if (x >= 0 && x < width && y >= 0 and y < height)
